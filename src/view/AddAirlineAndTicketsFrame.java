...
public class AddAirlineAndTicketsFrame extends JFrame implements ActionListener {
    Admin ADMIN;
    Runnable runnable;
    ...
    private class ButtonPanel extends JPanel {
        public ButtonPanel() {
            ...
            confirmButton.addActionListener(AddAirlineAndTicketsFrame.this);
            add(confirmButton, new GBC(0, 0).setInsets(10, 10, 10, 10).setIpad(200, 30));
        }
    }

    public AddAirlineAndTicketsFrame(Admin ADMIN, Runnable runnable) {
        this.ADMIN = ADMIN;
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == confirmButton) {
            ...
            var newAirline = new Airline(company, flightType, flightNumber, takeOffTime, arrivalTime, departure,
                    destination);
            ...
            if (AdminDao.addNewAirline(newAirline)) {
                int maxFid = AdminDao.getMaxFid();
                var newTicket = new Ticket(maxFid, economySeatNum, economyPrice, economyDiscount, businessSeatNum,
                        businessPrice,
                        businessDiscount, firstSeatNum, firstPrice, firstDiscount);
                if (AdminDao.addNewTicket(newTicket)
                        && AdminDao.addNewSeats(maxFid, economySeatCodes, businessSeatCodes, firstSeatCodes)) {
                    JOptionPane.showMessageDialog(this, "添加航班和机票信息成功", "成功", JOptionPane.INFORMATION_MESSAGE);
                    runnable.run();
                    dispose();
                } else {
                    JOptionPane.showMessageDialog(this, "添加机票信息失败", "错误", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "添加航班信息失败", "错误", JOptionPane.ERROR_MESSAGE);
            }
        }
    }

    public static void main(String[] args) {
        new AddAirlineAndTicketsFrame(new Admin(1, "admin", "admin"), () -> {
        });
    }
}
